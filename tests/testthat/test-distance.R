...
test_that("conv_reduce_all returns a valid distance for example-1", {
  ...
  expect_equal(
    as.numeric(format(conv_reduce_all(midata, 1, cosine_dist, min, get_middle_met_matrix = F)[2, 1], digits = 4)),
    as.numeric(format(1.101622e-02, digits = 4))
  )

  # for a vs d, we take maximum of a*b vs d and a*c vs d
  expect_equal(
    conv_reduce_all(midata, 1, cosine_sim, max, F, which.max)[4, 2], 0.9949405,
    tolerance = 1e-7
  )
  ...
  expect_equal(
    conv_reduce_all(midata, 1, cosine_sim, max, F, which.max)[2, 1], 0.9889838,
    tolerance = 1e-7
  )
...
test_that("conv_reduce_all returns a valid middle metabolite matrix for example-1", {
  ...
  c_diff <- matrix(
    abs(expand.grid(midata$peak_n_atoms, midata$peak_n_atoms)[, 1] - expand.grid(midata$peak_n_atoms, midata$peak_n_atoms)[, 2]),
    5, 5
  )
...
test_that("conv_reduce_all returns a valid distance for example-2", {
  ...
  expect_equal(
    as.numeric(sprintf(conv_reduce_all(midata, 1, cosine_dist, min, get_middle_met_matrix = F)[2, 1], fmt = "%#.5f")),
    0.00506
  )

  # for c vs a, we take maximum of c*b vs a and c*e vs a
  expect_equal(
    conv_reduce_all(midata, 1, cosine_sim, max, F, which.max)[4, 2], 0.4587568,
    tolerance = 1e-7
  )
  # test that conv_reduce_all returns a symmetric matrix
  expect_equal(
  ...
  # for a vs b, we get the similarity a*a vs b
  expect_equal(
    conv_reduce_all(midata, 1, cosine_sim, max, F, which.max)[2, 1], 0.9949405,
    tolerance = 1e-7
  )
  # for a vs c, we get a*a vs c which is NA
  expect_true(is.na(conv_reduce_all(midata, 1, cosine_sim, max, F, which.max)[3, 1]))
